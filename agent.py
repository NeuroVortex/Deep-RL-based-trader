...
class Agent:
    def __init__(self, state_size, is_eval=False, model_name=""):
        ...
        self.model = models.load_model("models/" + model_name) if is_eval else self.create_model()

    # Create deep learning model structure
    def create_model(self):
        model = models.Sequential()
        model.add(layers.Dense(units=32, input_dim=self.state_size, activation="relu"))
        ...
        for state, action, reward, next_state, done in mini_batch:
            if done:
                ...
                target = reward + self.gamma * np.amax(
                    next_q_values)  # ToDo: update target q_value using Bellman equation
    ...
    def sell(self, price_data):
        ...
        print("Sell: {0} | Profit: {1}".format(self.format_price(price_data),
